...
contract StockToken is ERC20, Ownable, ReentrancyGuard, IStockToken {
    ...
    /// @dev Access Modifier which specifies that the method can only be called by the Reflection wallet
    modifier onlyRefWalletAddress() {
        require(msg.sender == refWalletAddress, "Only ref wallet");
        _;
    }

    /// @dev Access Modifier which specifies that the method can only be called by the RUSD contract, which is the Reflection stablecoin
    ...
    /// @dev    Mint the StockToken. This is called to mint the stock token into the user's wallet when they buy a stock token on the Reflection platform.
    /// @param to (address)
    ...
    /// @dev    Burn the StockToken. This is called to burn the stock token out of the user's wallet when they sell a stock token on the Reflection platform.
    /// @param from (address)
    /// @param _amount (uint256)
